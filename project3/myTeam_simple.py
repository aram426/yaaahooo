...
class OffensiveAgent(ReflexAgent):
    ...
    def getWeights(self, gameState, action):
        myState = self.getSuccessor(gameState, action).getAgentState(self.index)
        ...
        # Dynamic weights based on situation
        if myState.numCarrying >= 3:
            # Carrying food → prioritize return
            weights['returnHome'] = -10
            weights['distanceToFood'] = 0

        # If ghost nearby, flee
        if 'ghostDistance' in self.getFeatures(gameState, action):
            ghost_dist = self.getFeatures(gameState, action)['ghostDistance']
            if ghost_dist < 5:
                weights['ghostDistance'] = 2  # Positive = stay away
                weights['distanceToFood'] = 0  # Ignore food
...
class DefensiveAgent(ReflexAgent):
    ...
    def getWeights(self, gameState, action):
        return {
            ...
            'invaderDistance': -10, # Chase invaders
            'stop': -100,           # Don't stop
